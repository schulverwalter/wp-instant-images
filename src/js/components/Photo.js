...
export default function Photo(props) {
	...
	const { provider, wpBlock = false, mediaModal = false, blockSidebar = false, clientId } = usePluginContext();

	const { id, id: filename, permalink, title, alt, caption, attribution, extension = 'jpg', likes, dimensions, urls, user } = result;
	...
	const likeDisplay = parseInt(likes) === 1 ? __('Like', 'instant-images') : __('Likes', 'instant-images');

	const { attribution_hook = false, auto_attribution = false } = instant_img_localize;

	// Get the attribution text.
	const imageAttribution = attribution_hook ? generateAttribution(attribution_hook, provider, permalink, user) : attribution;
			...
			<div className={classNames('photo-wrap', `photo-${status}`)}>
				<div className="img-wrap">
					...
					<div className="photo-controls">
						<a target="_blank" className="user fade" href={user_url} rel="noopener noreferrer" title={`${instant_img_localize.view_all} @ ${user_name}`}>
							<div className="user-wrap">
								{user_photo?.length > 0 && <img className="user-wrap--photo" src={user_photo} alt="" />}
								{user_name}
							</div>
						</a>
				...
				<div className="edit-screen" tabIndex="0" ref={editScreen}>
					...
					<div className="edit-screen--controls">
						<button type="button" className="button button-secondary" onClick={(e) => cancelEdit(e)}>
							{instant_img_localize.cancel}
						</button>{' '}
						&nbsp;
						<button type="button" className="button button-primary" onClick={() => uploadNow()}>
							{wpBlock ? instant_img_localize.insert_into_post : instant_img_localize.upload_now}
						</button>
					</div>
				</div>
